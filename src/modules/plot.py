...
def plot(history, title):
    ...
    global_metrics_centralised = history.metrics_centralized
    rounds = [data[0] for data in global_metrics_centralised["accuracy"]]
    acc = [100.0 * data[1] for data in global_metrics_centralised["accuracy"]]
    precision = [100.0 * data[1] for data in global_metrics_centralised["precision"]]
    recall = [100.0 * data[1] for data in global_metrics_centralised["recall"]]

    plt.plot(rounds, acc, color="blue", label="Accuracy")
    plt.plot(rounds, precision, color="green", label="Precision")
    plt.plot(rounds, recall, color="orange", label="Recall")
    plt.grid()
    plt.ylabel("Metrics (%)")
    plt.xlabel("Round")
    plt.title(title)
...
def smooth_plot(data, title, path, smoothing_window=5):
    ...
    global_metrics_centralised = data.metrics_centralized
    rounds = [data[0] for data in global_metrics_centralised["accuracy"]]
    acc = [data[1] for data in global_metrics_centralised["accuracy"]]
    precision = [data[1] for data in global_metrics_centralised["precision"]]
    recall = [data[1] for data in global_metrics_centralised["recall"]]

    # Apply smoothing
    if smoothing_window > 1:
        acc_smooth = moving_average(acc, smoothing_window)
        precision_smooth = moving_average(precision, smoothing_window)
        recall_smooth = moving_average(recall, smoothing_window)
        round_smooth = rounds[:len(acc_smooth)]
    else:
        acc_smooth = acc
        precision_smooth = precision
        recall_smooth = recall
        round_smooth = rounds
    ...
    with open(csv_path, mode='w', newline='') as file:
        ...
        writer.writerow(["Round", "Accuracy", "Precision", "Recall"])
        writer.writerows(zip(round_smooth, acc_smooth, precision_smooth, recall_smooth))

    print(f"Results saved to {csv_path}")

    plt.plot(round_smooth, acc_smooth, color="blue", label="Accuracy")
    plt.plot(round_smooth, precision_smooth, color="green", label="Precision")
    plt.plot(round_smooth, recall_smooth, color="orange", label="Recall")
    plt.grid()
    plt.xlim(left=0)
...
# untill here is plotting and saving 'accuracy' only code
# ===================================================================
